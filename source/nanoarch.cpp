...
//void die(const std::string& msg, const std::string& data) {
    char buffer[4096];
...
void video_refresh(const void *data, unsigned width, unsigned height, unsigned pitch) {
	...
	if (data) {
		...
		//for (unsigned int i = 0; i < image.width(); i=i+3)
		//{
		//	for (unsigned int j = 0; j < image.height(); j=j+3)
		//	{
		//		image.set_pixel(i,j,(data[i*j]),(data[(i+1)*j]),(data[(i+2)*j));
		//	}
		//}
...
void audio_init(int frequency) {
	...
	if ((err = snd_pcm_open(&g_pcm, "default", SND_PCM_STREAM_PLAYBACK, 0)) < 0)
		die("Failed to open playback device: " +std::string(snd_strerror(err)));

	err = snd_pcm_set_params(g_pcm, SND_PCM_FORMAT_S16, SND_PCM_ACCESS_RW_INTERLEAVED, 2, frequency, 1, 64 * 1000);

	if (err < 0)
		die("Failed to configure playback device: " + std::string(snd_strerror(err)));
}
...
void core_load(const char *sofile) {
	...
	if (!g_retro.handle)
		die("Failed to load core: " + std::string(dlerror()));
...
void core_load_game(const char *filename) {
	...
	if (!file)
	    die("Failed to load content '" + std::string(filename) + "' : " + std::string(strerror(errno)));
...
int main(int argc, char *argv[]) {
	if (argc < 3)
		die("usage: " + std::string(argv[0]) + "  <core> <game>");
